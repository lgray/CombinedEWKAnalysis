...
### builds the cross section tables for WW / WZ / Wgamma TGCs
### (Hagiwara & Zeppenfeld like)
class ChargedTGCBuilder:
    ...
    def makeXS(self,process, energy='7TeV'):
        ...
        if process == "WW": self.textToSpline("ATGC_XS_WW_"+energy, os.path.join(self.xspath, energy+"-WW.txt") );
        if process == "WZ": self.textToSpline("ATGC_XS_WZ_"+energy, os.path.join(self.xspath, energy+"WZ.txt") );
        if process == "Wgamma": self.textToSpline("ATGC_XS_Wgamma_"+energy, os.path.join(self.xspath, energy+"-Wgamma.txt") );
